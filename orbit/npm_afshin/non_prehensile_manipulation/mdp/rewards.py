...
def imitation(env: RLTaskEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    #robot is an attribute of interactiveSceneCfg and is an articulationCfg
    """Penalize z-axis base linear velocity using L2-kernel."""
    # extract the used quantities (to enable type-hinting)
    asset: Articulation = env.scene[asset_cfg.name]
    #According to this I should do env.scene.articulations["robot"] to access to the articulation Class
    """I would like that return a vector (N,12)"""
    #Pass them with expand into the init
    error_m1 = env.m1_joint_pos_extended[:, env.index_imit] - asset.data.joint_pos[:,0]  #joint_pos is (num_envs, num_joints)
    error_m2 = env.m2_joint_pos_extended[:, env.index_imit] - asset.data.joint_pos[:,1]

    sigma_square = 0.4*0.4
    env.index_imit +=1
    if(env.index_imit >= 1818):
           env.index_imit = 1
    #print("imitation term: ", torch.exp(-2*(1/sigma_square)*(error_m1*error_m1 + error_m2*error_m2)))

    return torch.exp(-2*(1/sigma_square)*(error_m1*error_m1 + error_m2*error_m2))
...
def ang_vel_x_l2(env: RLTaskEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    """Penalize xy-axis base angular velocity using L2-kernel."""
    # extract the used quantities (to enable type-hinting)
    asset: RigidObject = env.scene[asset_cfg.name]
    #this matrix is [num_envs, 3] where 3 are the angular velocities 
