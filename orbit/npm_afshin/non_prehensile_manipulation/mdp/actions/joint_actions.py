...
class JointPositionAction(JointAction):
    ...
    def __init__(self, cfg: actions_cfg.JointPositionActionCfg, env: BaseEnv):
        ...
        m1_joint_pos_extended = env.m1JointPos.expand(self.num_envs, -1)
        m2_joint_pos_extended = env.m2JointPos.expand(self.num_envs, -1)     

        if cfg.use_default_offset:
            self._offset = self._asset.data.default_joint_pos[:, self._joint_ids].clone()
            self._offset = torch.column_stack((m1_joint_pos_extended[:,env.index_imit],
                                               m2_joint_pos_extended[:,env.index_imit],
